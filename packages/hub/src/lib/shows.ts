...
export async function updateShow(id: string, input: ShowUpdate): Promise<{ show?: Show; error?: string }> {
  ...
  // Try progressively simpler payloads if optional columns are missing in the DB
  const attempts: Array<Record<string, unknown>> = [
    row,
    (({ gallery_paths, ...r }) => r)(row),
    (({ gallery_paths, rating, tags, ...r }) => r)(row),
    (({ gallery_paths, rating, tags, updated_at, ...r }) => r)(row),
  ];
  ...
  for (let i = 0; i < attempts.length; i++) {
    ...
    if (!error) {
      return { show: mapRowToShow({ ...(data as ShowRow), gallery_paths: (row.gallery_paths as string[] | undefined) ?? [] }) };
    }

    const isLast = i === attempts.length - 1;
    if (isLast || !isMissingColumnError(error)) {
