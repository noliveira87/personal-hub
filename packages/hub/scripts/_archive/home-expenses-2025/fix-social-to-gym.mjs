...
try {
  console.log('🔄 Updating "Segurança Social" from social-security to gym...');
  
  const { data, error: updateError } = await supabase
    .from('home_expenses_transactions')
    .update({ category: 'gym' })
    .eq('category', 'social-security')
    .like('name', '%Segurança Social%')
    .select('date, name, category, amount, type');

  if (updateError) throw updateError;

  console.log(`✅ Updated ${data.length} transactions to gym category`);
